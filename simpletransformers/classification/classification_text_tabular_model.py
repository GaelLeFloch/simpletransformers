...
from simpletransformers.classification.transformer_models.flaubert_model import FlaubertForSequenceClassification, FlaubertModel

class SequenceSummary(nn.Module):
    r""" Compute a single vector summary of a sequence hidden states """

    def __init__(self, config, xs_dim: int):
        super().__init__()

        num_classes = config.num_labels
        self.first_dropout = nn.Dropout(config.summary_first_dropout)
        self.summary = nn.Linear(config.hidden_size + xs_dim, num_classes)
    ...
    def forward(self, hidden_states, cls_index=None):
        ...
        output = self.first_dropout(hidden_states)
        output = self.summary(output)

        return output

class FlaubertTabularClassification(FlaubertModel):
    r"""
    ...
    def __init__(self, config, xs_dim, weight=None):
        super(FlaubertTabularClassification, self).__init__(config)
        self.num_labels = config.num_labels
        self.weight = weight

        self.transformer = FlaubertModel(config)
        # self.tabular_model = "something"
        self.sequence_summary = SequenceSummary(config, xs_dim) # Change hidden size to hs of transformer + tab model
    ...
    def forward(
        ...
        # tab_output = self.tabular_model(xs)

        embedding_output = torch.cat([transformer_outputs[0][:,0], xs], 1)

        logits = self.sequence_summary(embedding_output)
...
class ClassificationTabModel:
    def __init__(
        ...
        config_class, model_class, tokenizer_class = (FlaubertConfig,  FlaubertTabularClassification, FlaubertTokenizer)
        if num_labels:
    ...
    def train_model(
        ...
        **kwargs,
        ...
        if isinstance(train_df, str) and self.args.lazy_loading:
            ...
            # Add Xs
            col_Xs = list(train_df.columns[train_df.columns.str.contains('x_')])
            torch_xs = torch.tensor(train_df[col_Xs].values, dtype=torch.long)

            train_dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, torch_xs)
    ...
    def evaluate(
        ...
        if isinstance(eval_df, str) and self.args.lazy_loading:
            ...
            if args.sliding_window:
                ...
                # Add Xs
                X_s = list(eval_df.columns[eval_df.columns.str.contains('x_')])
                torch_xs = torch.tensor(eval_df[X_s].values, dtype=torch.long)
    ...
    def load_and_cache_examples(
        ...
        """
        Converts a list of InputExample objects to a tuple containing InputFeatures. Caches the InputFeatures.
        ...
        if args.sliding_window and evaluate:
            return (all_input_ids, all_input_mask, all_segment_ids, all_label_ids), window_counts
        else:
    ...
    def _get_inputs_dict(self, batch):
        if isinstance(batch[0], dict):
            ...
            inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3], 'xs': batch[4]}
